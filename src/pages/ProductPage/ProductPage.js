...
import { FiShare2, FiMaximize2, FiTruck, FiTag, FiBox, FiCopy, FiX, FiChevronLeft, FiChevronRight, FiInfo } from 'react-icons/fi';
import { IoStar, IoStarOutline } from 'react-icons/io5';
import { FaFacebookF, FaTwitter, FaPinterestP, FaHeart, FaRegHeart } from 'react-icons/fa';
...
const ProductPage = () => {
  ...
  const handleWishlistClick = (e) => {
    ...
    if (wishlistItems.includes(currentProductId)) {
      dispatch(removeFromWishlistThunk({ userId, productId: currentProductId }))
        .unwrap()
        .then(() => showToast("Product removed from your Wishlist"))
        .catch((err) => alert("Error removing from wishlist: " + err));
    } else {
      dispatch(addToWishlistThunk({ userId, productId: currentProductId }))
        .unwrap()
            ...
            <div className="product-details-section">
              ...
              <div className="product-price-row">
                <div className="product-price" style={{ display: 'flex', alignItems: 'baseline', gap: '10px', flexWrap: 'wrap' }}>
                  <span style={{ whiteSpace: 'nowrap' }}>Rs. {Number(product.price).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  {Number(product.mrp) > Number(product.price) && (
                    <span style={{ textDecoration: 'line-through', color: '#888', fontSize: '18px', fontWeight: 'normal', whiteSpace: 'nowrap' }}>
                      Rs. {Number(product.mrp).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                    </span>
                  )}
                </div>

                <div className="product-meta-links" style={{ display: 'flex', gap: '15px' }}>
                  <div className="meta-link" onClick={handleShare} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}><FiShare2 /> Share</div>
                  <div className="meta-link" onClick={handleWishlistClick} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                    {wishlistItems.includes(product?.id || id) ? (
                      <>
                        <FaHeart color="red" /> Wishlisted
                      </>
                    ) : (
                      <>
                        <FaRegHeart /> Wishlist
